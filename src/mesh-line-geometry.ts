...
export class MeshLineGeometry extends BufferGeometry {
    ...
    setPoints(
        ...
        if (this.pointsAreEqual(0, pointCount - 1)) {
            getIndex = (pointCount - 2) * ITEM_SIZE;
        ...
        for (let j = 0; j < pointCount; j++) {
            ...
            if (wcbChanged || sizeChanged) {
                // widths
                width = this.widthCallback ? this.widthCallback(j / (pointCount - 1)) : 1;
        ...
        if (this.pointsAreEqual(pointCount - 1, 0)) {
            getIndex = 1 * ITEM_SIZE;
            x = this.#positions[getIndex + 0];
            y = this.#positions[getIndex + 1];
            z = this.#positions[getIndex + 2];
        } else {
            getIndex = (pointCount - 1) * ITEM_SIZE;
            x = this.#positions[getIndex + 0];
            y = this.#positions[getIndex + 1];
            z = this.#positions[getIndex + 2];
        }
    ...
    makeNewBuffers(pointCount: number) {
        ...
        this.#attributes = {
            position: new BufferAttribute((this.#positions = new Float32Array(pointCount * ITEM_SIZE)), 3),
            previous: new BufferAttribute((this.#previous = new Float32Array(pointCount * ITEM_SIZE)), 3),
            next: new BufferAttribute((this.#next = new Float32Array(pointCount * ITEM_SIZE)), 3),
            ...
            index: new BufferAttribute((this.#indices = new Uint16Array((pointCount - 1) * ITEM_SIZE)), 1),
        };
    ...
    private pointsAreEqual(pointIndexA: number, pointIndexB: number) {
        const actualIndexA = pointIndexA * ITEM_SIZE;
        const actualIndexB = pointIndexB * ITEM_SIZE;
        return (
    ...
    advance(position: Vector3) {
        ...
        memcpy(positions, ITEM_SIZE, positions, 0, l - ITEM_SIZE);
        ...
        memcpy(positions, ITEM_SIZE, next, 0, l - ITEM_SIZE);
        next[l - 6] = position.x;
        next[l - 5] = position.y;
        next[l - 4] = position.z;
        next[l - 3] = position.x;
...
function isVector3Array(array: Vector3[] | WritableArrayLike<number>): array is Vector3[] {
    return !!(array.length > 0 && array[0] instanceof Vector3);
}
